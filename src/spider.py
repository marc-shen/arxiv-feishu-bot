...
def main():
    ...
    card_content = {
        ...
        "header": {
            "template": "blue",
            "title": {"content": "今日论文推荐", "tag": "plain_text"},
        },
    }
    ...
    for idx, paper in enumerate(paper_list):
        ...
        c.execute('INSERT INTO PAPER (NAME) VALUES ("{}")'.format(paper["title"]))
    ...
    if len(paper_list) == 0:
        card_content["elements"].append(
            {
                "tag": "div",
                "text": {"content": "No papers to follow today.", "tag": "lark_md"},
            }
        )
    card_content["elements"].append(
        {
            "actions": [
                {
                    "tag": "button",
                    "text": {"content": "查看今日全部论文", "tag": "plain_text"},
                    "type": "primary",
                    "url": NEW_SUB_URL,
                }
            ],
